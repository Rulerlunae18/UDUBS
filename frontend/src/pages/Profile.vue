...
onMounted(async () => {
  try {
    if (isAdmin.value) {
      ...
      // ----- REAL USER → get FakeUser slot -----

      // 1. load all fakeUsers (they already come substituted for this realUser)
      const resFu = await api.get(`/fakeusers`)

      // 2. find fakeUser assigned to this realUser (slot)
      const myFu = resFu.data.find(fu => fu.userId === auth.user.id)

      if (!myFu) {
        console.error("❌ FakeUser slot not found for user:", auth.user.id)
        return
      }
      ...
      // Fill visible fields  
      name.value = myFu.codename
      title.value = myFu.rank
      bio.value = myFu.bio
    }
  ...
  try {
    ...
    if (isAdmin.value) {
      ...
      if (res.data?.user) {
        ...
        auth.user.title = updated.title
        auth.user.bio = updated.bio
        if (updated.avatarUrl) {
          auth.user.avatarUrl = backendUrl(updated.avatarUrl)
        }
      }
      ...
      // =======================================
      // PUT → /realusers-self/me/avatar
      // (именно этот маршрут предназначен для USER)
      // =======================================
      const res = await api.put(`/realusers-self/me/avatar`, fd, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })

      const newUrl = res.data?.avatarUrl
      if (newUrl) {
        profile.value.avatarUrl = backendUrl(newUrl)
