...
// ------------------------------------------------------------
//  Helpers — generate dynamic BIO from GameProfile
// ------------------------------------------------------------
...
// ------------------------------------------------------------
//  GET /posts/:id — получить пост с подменой автора при необходимости
// ------------------------------------------------------------
// ------------------------------------------------------------
//  GET /posts/:id — получить пост с подменой автора при необходимости
// ------------------------------------------------------------
async function getPost(req, res) {
  try {
    ...
    // =====================================================================================
    // ⭐ REAL USER → подменяем только если пост принадлежит его FAKE SLOT
    // =====================================================================================
    if (user.type === "real") {
      const real = user.realUser;  // уже добавлено в auth.js

      // real.userId — это systemUser.id, к которому привязан fakeUser
      const systemUserId = real.userId;

      // fakeAuthor.userId — ID system user владельца данного fakeUser
      if (post.fakeAuthor?.userId === systemUserId) {
        ...
        const substituted = {
          ...
          clearance: "D-13",
          bio: stats
            ? generateDynamicBio(stats)
            : "Autogenerated field operator."
        };
...
async function createPost(req, res) {
  try {
    ...
    let coverImage = req.file ? publicUrl(req.file.path) : null;
    let fileType = req.file?.mimetype || null;
...
async function updatePost(req, res) {
  try {
    ...
    if (req.files?.images?.length) {
      ...
      await prisma.postImage.createMany({ data: imgs });
    }
